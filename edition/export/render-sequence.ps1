...
# Motion vectors from analyse-stabilisation.ps1, if a clip has been analysed.
$stabilisation = @{}
foreach ($m in $index.media) {
    if ($m.PSObject.Properties.Name -contains 'stabilisation' -and $m.stabilisation) {
        $stabilisation[$m.name] = Join-Path $mediaDir `
            (Join-Path 'stabilisation' (Split-Path -Leaf $m.stabilisation))
    }
}
...
foreach ($v in $videoOps) {
    ...
    # Stabilisation first: the vectors describe the camera's own motion, so
    # anything that moves or scales the picture beforehand would be
    # correcting against a frame that no longer matches the measurement.
    $stabParam = Get-Param $effects 'Stabilise' 'Smoothing'
    if ($null -ne $stabParam -and $stabilisation.ContainsKey($v.clip.media)) {
        $smoothing = [int][Math]::Round($stabParam.value)
        if ($smoothing -gt 0) {
            $trf = ($stabilisation[$v.clip.media] -replace '\', '/') -replace ':', '\:'
            [void]$filter.Append("vidstabtransform=input='${trf}':smoothing=${smoothing}:crop=black,")
        }
    } elseif ($null -ne $stabParam) {
        Write-Warning "$($v.clip.label): stabilisation asked for but not analysed; run analyse-stabilisation.ps1"
    }
