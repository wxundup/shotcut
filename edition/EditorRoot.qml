/*
 * The EdiTogether interface.
 *
 * An Item rather than a window, so it can be shown standalone (Shell.qml
 * wraps it in an ApplicationWindow) or hosted inside the application in a
 * QQuickWidget, which requires an Item root. Nothing here depends on being
 * a top-level window.
 */
